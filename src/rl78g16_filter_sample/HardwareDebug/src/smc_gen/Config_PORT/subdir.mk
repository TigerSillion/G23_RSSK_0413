...
src/smc_gen/Config_PORT/%.obj: ../src/smc_gen/Config_PORT/%.c 
	...
	ccrl -subcommand="src\smc_gen\Config_PORT\cDepSubCommand.tmp" -o "$(@:%.obj=%.d)" -MT="$(@:%.obj=%.obj)" -MT="$(@:%.obj=%.d)" -msg_lang=japanese "$<"
	@echo src\smc_gen\Config_PORT\cSubCommand.tmp=
	@sed -e "s/^/ /" "src\smc_gen\Config_PORT\cSubCommand.tmp"
	ccrl -subcommand="src\smc_gen\Config_PORT\cSubCommand.tmp" -msg_lang=japanese -o "$(@:%.d=%.obj)" "$<"
