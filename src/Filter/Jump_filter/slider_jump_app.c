...
/* 不同滤波器预设 */
static const jump_preset_t g_presets[] = {
    {
        .name = "标准",
        .off_count_limit = 3,
        .history_size = 3,
...
/* 全局变量 */
static uint8_t g_current_preset = 0;  // 默认使用"标准"预设
...
/* 滤波器控制块和配置 */
static slider_jump_ctrl_t g_filter_ctrl;
static slider_jump_cfg_t g_filter_cfg;
...
const char* slider_jump_switch_preset(void)
{
    ...
    // 更新滤波器配置
    g_filter_cfg.off_count_limit = g_presets[g_current_preset].off_count_limit;
    g_filter_cfg.history_size = g_presets[g_current_preset].history_size;
    g_filter_cfg.a_coef = g_presets[g_current_preset].a_coef;
    g_filter_cfg.b_coef = g_presets[g_current_preset].b_coef;
    g_filter_cfg.predict_enable = g_presets[g_current_preset].predict_enable;
    g_filter_cfg.adaptive_enable = g_presets[g_current_preset].adaptive_enable;
    
    // 重新初始化滤波器
    slider_jump_init(&g_filter_ctrl, &g_filter_cfg);
...
void slider_jump_set_custom_coef(uint8_t a_value, uint8_t b_value)
{
    ...
    // 更新滤波器系数
    slider_jump_update_coef(&g_filter_ctrl, a_coef, b_coef);
...
uint16_t slider_jump_process(uint16_t raw_position)
{
    ...
    // 应用滤波器
    filtered_position = slider_jump_process(&g_filter_ctrl, raw_position, current_time);
...
{
    slider_jump_set_predict(&g_filter_ctrl, enable);
    
    if (g_debug_output)
    {
        sprintf(g_print_buffer, "预测功能: %s\n", enable ? "启用" : "禁用");
        DEBUG_PRINT(g_print_buffer);
    }
}
...
{
    slider_jump_set_adaptive(&g_filter_ctrl, enable);
    
    if (g_debug_output)
    {
        sprintf(g_print_buffer, "自适应系数: %s\n", enable ? "启用" : "禁用");
        DEBUG_PRINT(g_print_buffer);
    }
}
...
void slider_jump_set_off_count(uint8_t count)
{
    if (count < 1 || count > 10)
    {
        if (g_debug_output)
        {
            DEBUG_PRINT("错误: OFF阈值必须在1-10之间\n");
        }
        return;
    }
    
    g_filter_cfg.off_count_limit = count;
    slider_jump_init(&g_filter_ctrl, &g_filter_cfg);
    
    if (g_debug_output)
    {
        sprintf(g_print_buffer, "设置OFF阈值: %d\n", count);
        DEBUG_PRINT(g_print_buffer);
    }
}
...
uint16_t slider_jump_app_process(void)
{
    ...
    // 处理滑条数据
    filtered_position = slider_jump_process(slider_position);
...
{
    // 获取滤波后的滑条位置
    uint16_t slider_position = slider_jump_app_process();
