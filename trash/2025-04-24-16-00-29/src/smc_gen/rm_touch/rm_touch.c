                          ...
                          uint8_t slider_id)
{
    ...
    for (loop = 0; loop < num_elements && loop < 16; loop++) {
        g_touch_slider_debug.raw_data[loop] = slider_data[loop];
    }

    /* Store previous position */
    g_touch_slider_debug.prev_position = g_touch_slider_debug.position;
    ...
    /* Store debug info */
    g_touch_slider_debug.position = slider_rpos;

    /* Print debug info for analysis */
    printf("SLIDER,%u,%u", g_touch_slider_debug.position, g_touch_slider_debug.prev_position);
    for (loop = 0; loop < num_elements && loop < 16; loop++) {
        printf(",%u", g_touch_slider_debug.raw_data[loop]);
    }
    printf("\n");
