...
/* Slider algorithm selection */
typedef enum e_touch_slider_algo
{
    TOUCH_SLIDER_ALGO_DEFAULT = 0,  // Default algorithm in rm_touch.c
    TOUCH_SLIDER_ALGO_PATCH,        // Modified algorithm in rm_touch_patch
    TOUCH_SLIDER_ALGO_CUSTOM        // Reserved for future custom algorithm
} touch_slider_algo_t;

/* Global variable for algorithm selection */
extern volatile touch_slider_algo_t g_touch_slider_algo;

/* Define which implementation to use */
#ifndef TOUCH_CFG_USE_PATCH_VERSION
#define TOUCH_CFG_USE_PATCH_VERSION    (0)    // 0: Use original version, 1: Use patch version
#endif

#if TOUCH_CFG_USE_PATCH_VERSION
#define TOUCH_IMPLEMENT_PREFIX patch_
#else
#define TOUCH_IMPLEMENT_PREFIX
#endif

/* Function name mapping */
#define TOUCH_CONCAT_(a, b) a ## b
#define TOUCH_CONCAT(a, b) TOUCH_CONCAT_(a, b)
#define TOUCH_IMPLEMENT(name) TOUCH_CONCAT(TOUCH_IMPLEMENT_PREFIX, name)

/* Slider debug info for linearity analysis */
typedef struct st_touch_slider_debug
{
    uint16_t position;          // Current position
    uint16_t prev_position;     // Previous position
    uint32_t timestamp;         // Timestamp in ms
    uint16_t raw_data[16];     // Raw sensor data for analysis
} touch_slider_debug_t;

extern touch_slider_debug_t g_touch_slider_debug;
...
#if TOUCH_CFG_SLIDER_ALGO == TOUCH_SLIDER_ALGO_PATCH
void touch_slider_decode_patch(touch_slider_info_t * p_sinfo, uint16_t * slider_data, uint8_t num_elements, uint8_t slider_id);
#endif

/* External variable declarations */
extern volatile uint32_t g_msCounter;  // System tick counter
